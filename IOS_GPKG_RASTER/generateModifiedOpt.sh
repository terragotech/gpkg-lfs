#!/bin/bash
set -u
# The notion of this script is generate GDALMake.opt file for different targets and preserve them in
# a folder for each build target
# So this GDALMake.opt will be copied to the target folder
# The builder must modify those file to get a minimal GDAL build
...
# Now check for the folder if not create a folder and copy the GDALMake.opt to the target
if [ ! -d "$arch" ]; then
  mkdir "$arch"
fi
cp ./GDALmake.opt ./$arch/GDALmake.opt 
