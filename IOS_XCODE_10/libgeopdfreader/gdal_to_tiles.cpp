...
void geopdf_clean_tmp_files_gtt(struct gdal_tiles *gt)
{
	char fpath[1024];
	char path[1024];
	fpath[0] = '\0';
	...
	if(NULL != gt->tmp_folder){
		...
		{
			//sprintf(rpath,”chmod 777 %s/mbtiles/%s_*.%s",gt->tmp_folder,gt->os_process_id,"png");
			//sprintf(fpath,"rm -f %s/mbtiles/%s_*.%s",gt->tmp_folder,gt->os_process_id,"png");
		}
		else
		{
			//sprintf(rpath,”chmod 777 %s/mbtiles/%s_*.%s",gt->tmp_folder,gt->os_process_id,"jpg");
...
void geopdf_remove_resources_gtt(struct gdal_tiles *gt)
{
...
int geopdf_clean_png_file(struct gdal_tiles *gt, char *file_name)
{
...
void geopdf_clear_all_elements(struct gdal_tiles *gt)
{
...
int geopdf_insert_element(struct gdal_tiles *gt, struct tile_exists_info *ptrElement){
	int result = MBGEN_SUCCESS;
...
int geopdf_try_to_use_existing_tile(struct gdal_tiles *gtiles,int tx,int ty,int tz){
...
int geopdf_check_for_children(struct gdal_tiles *gtiles,int tx,int ty,int tz)
{
	int x = 0;
	int y = 0;
...
//	char *tail;
	...
	for(y=2*ty;y<(2*ty+2);y++){
		for(x=2*tx;x<(2*tx+2);x++){
			tile_found = geopdf_try_to_use_existing_tile(gtiles,x,y,tz+1);
			
			tef.tx = x;
			tef.ty = y;
			tef.tz = tz+1;
			if(tile_found == 1){
				result = geopdf_insert_element(gtiles,&tef);
				if(result < 0){
					return result;
				}
...
#endif
int geopdf_write_overview_tile(struct gdal_tiles *gtiles,int tx,int ty,int tz,xyzzy *process_tile_info){
	...
	result = geopdf_check_for_children(gtiles,tx,ty,tz);
	...
	{
		geopdf_clear_all_elements(gtiles);
	...
	{
		result = geopdf_write_mbtiles_gtt(gtiles,tz,tx,ty,ptrFileBinData,file_size);
	}
	if(ptrFileBinData != NULL){
		free(ptrFileBinData);
		ptrFileBinData = NULL;
...
int geopdf_write_base_tile(struct gdal_tiles *gtiles,int tx,int ty,int tz,xyzzy *process_tile_info)
{
	int b_idx = 0;
	int result = 0;
	GDALRasterBandH hBand;
					...
					0,0,0) == CE_None)
					{
						...
						if(strcmp(gtiles->image_type,"png") == 0)
						{
							...
							if (papszOptions != NULL) 
							{
 							...
 							result = geopdf_clean_png_file(gtiles,file_name);
							if(result < 0)
							{
								return result;
							}
...
/// set_input_filename_gtt
/// Set the input file name in the gdal_tiles structure
int geopdf_set_input_filename_gtt(struct gdal_tiles *gtiles,char *filename){
	int result = MBGEN_SUCCESS;
...
void geopdf_geo_query(struct global_mercator *gm, struct gdal_tiles *gtiles,double ulx, double uly, double lrx, double lry,
	int *rx,int *ry,
...
*/
///Returns the maximum value of two double values
double geopdf_max_d_gtt(double a,double b)
{
	double c;
	if(a >= b)
	{
...
*/
///Returns the minimum value of two double values
double geopdf_min_d_gtt(double a,double b)
{
	double c;
	if(a <= b)
	{
...
int geopdf_gn_max_gtt(int a,int b){
	int max_value = 0;
	if(a >= b){
		max_value = a;
	}
...
*/
///Intitialize the gdal_tiles structure 
void geopdf_init_gdal_tiles_gtt(struct gdal_tiles *gt, char *pTMP, char *pUtid)
{
	gt->corner_tile = 0;
...
int geopdf_open_input_gtt(struct gdal_tiles *gt,struct global_mercator *gg){
	...
	if(gt->in_ds != NULL)
	{
		...
		if(raster_bands != 0)
		{
			...
			for(tz=0;tz<32;tz++)
			{
				...
				geopdf_MetersToTile(gg,gt->ominx,gt->ominy,tz,&tminx,&tminy);
				geopdf_MetersToTile(gg,gt->omaxx,gt->omaxy,tz,&tmaxx,&tmaxy);
				
				tminx = geopdf_gn_max_gtt(0,tminx);
				tminy = geopdf_gn_max_gtt(0,tminy);

				max_value = geopdf_gn_max_gtt( gt->image_width, gt->image_height);
				...
				gt->tminz = geopdf_ZoomForPixelSize(gg,gt->in_gt[1] * ((float)max_value / (float) gt->tileSize));
				gt->tmaxz = geopdf_ZoomForPixelSize(gg,gt->in_gt[1]);
...
*/
///Generate Metadata for writing into MBTiles
void geopdf_generate_metadata_gtt(struct gdal_tiles *gt,struct global_mercator *gg)
{

	double south_lat, west_lon;
	double north_lat, east_lon;

	geopdf_MetersToLatLon(gg,gt->ominx,gt->ominy,&south_lat,&west_lon);
	geopdf_MetersToLatLon(gg,gt->omaxx,gt->omaxy,&north_lat,&east_lon);

	south_lat = geopdf_max_d_gtt(-85.05112878, south_lat);
	west_lon = geopdf_max_d_gtt(-180.0,west_lon);
		
	north_lat = geopdf_min_d_gtt(85.05112878, north_lat);
	east_lon = geopdf_min_d_gtt(180.0,east_lon);
	...
	geopdf_TileLatLonBounds(gg,tminx,tminy,tz,&minLat,&minLon,&maxLat,&maxLon);
...
*/
///Remove distortions from the image edges
void geopdf_remove_distortions(struct gdal_tiles *gt,struct global_mercator *gg, int tx,int ty,int tz, TileInfo *ptrtf)
{
	TileInfo tf = *ptrtf;
	...
	geopdf_TileBounds(gg,tx,ty,tz,&b_min_x,&b_min_y,&b_max_x,&b_max_y);
	geopdf_geo_query(gg,gt,b_min_x,b_max_y,b_max_x,b_min_y,&rx1,&ry1,&rxsize1,&rysize1,&wx1,&wy1,&wxsize1,&wysize1,querysize);
	...
	if((tx == tf.tminx) && (ty == tf.tminy)){
		...
		else
		{
			...
			geopdf_TileBounds(gg,tx+1,ty,tz,&b_min_x,&b_min_y,&b_max_x,&b_max_y);
		...
		if(ty-1 > tf.tminy){
			...
			geopdf_TileBounds(gg,tx,ty-1,tz,&b_min_x,&b_min_y,&b_max_x,&b_max_y);
			//Query the neigbour tile
			geopdf_geo_query(gg,gt,b_min_x,b_max_y,b_max_x,b_min_y,&rx1,&ry1,&rxsize1,&rysize1,&wx1,&wy1,&wxsize1,&wysize1,querysize);
...
*/
///Generate Base tiles 
int geopdf_generate_base_tiles_gtt(struct gdal_tiles *gt,struct global_mercator *gg)
{
	...
	for(ty=tf.tmaxy;ty>tf.tminy-1;ty--)
	{
				for(tx=tf.tminx;tx<tf.tmaxx+1;tx++)
				{
					...
					geopdf_TileBounds(gg,tx,ty,tz,&b_min_x,&b_min_y,&b_max_x,&b_max_y);
					geopdf_geo_query(gg,gt,b_min_x,b_max_y,b_max_x,b_min_y,
					...
					double percentage = (((double)gt->current_tile_count)/((double)gt->total_tile_count)) * 0.45f;
					geopdf_create_status_file_progress(gt->process_id,percentage+0.55f);
					...
					result = geopdf_write_base_tile(gt,tx,ty,tz,&process_tile_info);
...
*/
///Set the MBTiles name 
int geopdf_set_mbtiles_name_gtt(struct gdal_tiles *gt,char *mbtiles_file_name)
{
	int result = MBGEN_SUCCESS;
...
*/
int geopdf_generate_overview_tiles_gtt(struct gdal_tiles *gt,struct global_mercator *gg)
{
	...
	for(tz=gt->tmaxz-1;tz>=gt->tminz;tz--)
	{
		...
		for(ty=tf.tmaxy;ty>tf.tminy-1;ty--)
		{
					...
					for(tx=tf.tminx;tx<tf.tmaxx+1;tx++)
					{
						...
						termination_status = geopdf_check_process_termination(gt->termFile);
						printf("%.f...",floor( ((double)gt->current_tile_count)/((double)gt->total_tile_count) *100 )); 
						result = geopdf_write_overview_tile(gt,tx,ty,tz,&process_tile_info);
#ifdef OS_WINDOWS_BUILD
					Sleep(250);
#else
					usleep(250000);
...
void geopdf_process(struct gdal_tiles *gt,struct global_mercator *gg){
	open_input(gt,gg);
...
*/
///Open the MBTiles file
int geopdf_open_mbtiles_gtt(struct gdal_tiles *gt)
{
	int result = MBGEN_SUCCESS;
...
int geopdf_create_table_mbtiles_gtt(struct gdal_tiles *gt)
{
	int result = MBGEN_SUCCESS;
...
*/
///Close MBTiles
int geopdf_close_mbtiles_gtt(struct gdal_tiles *gt)
{
	int result = MBGEN_SUCCESS;
...
*/
///Insert Meta data into metadata table in MBTiles
int geopdf_insert_metadata_mbtiles_gtt(struct gdal_tiles *gt)
{
	int result = MBGEN_SUCCESS;
...
int geopdf_write_mbtiles_gtt(struct gdal_tiles *gtiles, int tz, int tx, int ty, unsigned char *ptrBlobData, unsigned int file_size)
{
	int result = MBGEN_SUCCESS;
...
int geopdf_remove_existing_mbtiles_gtt(char *mbtile_file_name)
{
	int result = MBGEN_SUCCESS;
