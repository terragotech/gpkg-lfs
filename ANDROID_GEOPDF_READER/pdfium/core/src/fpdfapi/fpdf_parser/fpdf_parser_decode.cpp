        ...
        int nComps, int bpc, const CPDF_Dictionary* pParams)
{
	...
	if(curGet != NULL)
	{
		return curGet->GetFlateModule()->CreateDecoder(src_buf, src_size, width, height,
                                  ...
                                  FX_DWORD estimated_size, uint8_t*& dest_buf, FX_DWORD& dest_size)
{
	...
	if(curGet == NULL)
	{
		 CCodec_ModuleMgr *g_pCodecModule = new CCodec_ModuleMgr();
		 CPDF_ModuleMgr::Create();
		curGet = CPDF_ModuleMgr::Get();
    		curGet->SetCodecModule(g_pCodecModule);
		
	}
	if(curGet != NULL)
	{
		return curGet->GetFlateModule()->FlateOrLZWDecode(bLZW, src_buf, src_size,
...
void FlateEncode(const uint8_t* src_buf, FX_DWORD src_size, uint8_t*& dest_buf, FX_DWORD& dest_size)
{
	...
	if(curGet != NULL)
	{
		CCodec_ModuleMgr* pEncoders = curGet->GetCodecModule();
		    if (pEncoders) {
