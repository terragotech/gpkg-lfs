...
static FX_DWORD _EmbeddedCharcodeFromUnicode(const FXCMAP_CMap* pEmbedMap, int charset, FX_WCHAR unicode)
{
 ...
 //const FX_WORD* pCodes = pFontGlobals->m_EmbeddedToUnicodes[charset].m_pMap;
	const FX_WORD* pCodes = NULL;
	CPDF_ModuleMgr *curGet = CPDF_ModuleMgr::Get();
	if(curGet != NULL)
	{
		IPDF_PageModule *curPM = curGet->GetPageModule();
		if(curPM != NULL)
		{
			CPDF_FontGlobals* pFontGlobals = curPM->GetFontGlobals();
			if(pFontGlobals != NULL)
			{
				pCodes = pFontGlobals->m_EmbeddedToUnicodes[charset].m_pMap;
				if (pCodes == NULL) {
					return 0;
				    }
...
FX_BOOL CPDF_CIDFont::_Load()
{
	...
	if(curGet != NULL)
	{
		...
		if(curPM != NULL)
		{
			...
			if(pFontGlobals != NULL)
			{
				m_pCMap = pFontGlobals->m_CMapManager.GetPredefinedCMap(cmap,m_pFontFile && m_bType1);
			}
		}
	}
	...
	if(curGet != NULL)
	{
		...
		if(curPM != NULL)
		{
			...
			if(pFontGlobals != NULL)
			{
				m_pCID2UnicodeMap = pFontGlobals->m_CMapManager.GetCID2UnicodeMap(m_Charset,
                            m_pFontFile == NULL && (m_pCMap->m_Coding == CIDCODING_CID || pCIDFontDict->KeyExist(FX_BSTRC("W"))));
			}
		}
	}
	}//End of  above if condition
    if (m_Font.GetFace()) {
        if (m_bType1) {
...
FX_BOOL CPDF_CIDFont::LoadGB2312()
{
	...
	if(curGet != NULL)
	{
		...
		if(curPM != NULL)
		{
			...
			if(pFontGlobals != NULL)
			{
				m_pCMap = pFontGlobals->m_CMapManager.GetPredefinedCMap(FX_BSTRC("GBK-EUC-H"), FALSE);
				m_pCID2UnicodeMap = pFontGlobals->m_CMapManager.GetCID2UnicodeMap(m_Charset, FALSE);
			}
		}
	}
    if (!IsEmbedded()) {
